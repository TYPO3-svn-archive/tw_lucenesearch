 ...
 */
class Tx_TwLucenesearch_Service_Lucene extends t3lib_svbase implements t3lib_Singleton {
	/**
	 * Index directory
	 * 
	 * @var string
	 ...
	 * 
	 * @param Tx_TwLucenesearch_Domain_Model_Document $document		Document
	 * @return boolean												Success
	 */
	public function add(Tx_TwLucenesearch_Domain_Model_Document $document) {
	 ...
	 * @return Tx_TwLucenesearch_Domain_Model_Document				Requested document
	 */
	public function get($reference, &$hit = null) {
		...
		if ($hit instanceof Zend_Search_Lucene_Search_QueryHit) {
			return Tx_TwLucenesearch_Domain_Model_Document::cast($hit->getDocument());
		} else {
			$hit				= null;
			return null;
		}
	 ...
	 * @return Tx_TwLucenesearch_Domain_Model_QueryHits				Query hits
	 */
		...
		if (strlen($searchTerm)) {
			...
			if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['tw_lucenesearch']['search-rewrite-hooks'])) {
				...
				foreach($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['tw_lucenesearch']['search-rewrite-hooks'] as $rewriteHook)	{
					t3lib_div::callUserFunction($rewriteHook, $params, $this);
				}
			}
			...
			try {
				$highlight		= intval(Tx_TwLucenesearch_Utility_Indexer::indexConfig($GLOBALS['TSFE'], 'search.highlightMatches'));
				
				// Construct the search request
				$query			= $this->query($searchTerm);
				
				// Run the search an collect the results
				$hits			= new Tx_TwLucenesearch_Domain_Model_QueryHits($this->_index(), $query, $tokenTerms, (boolean)$highlight);
	...
	public function query($searchTerm) {
		...
		if (strlen(trim($searchTerm))) {
			...
			if (Tx_TwLucenesearch_Utility_Indexer::indexConfig($GLOBALS['TSFE'], 'search.restrictByLanguage')) {
			...
			$rootlinePids	= Tx_TwLucenesearch_Utility_Indexer::indexConfig($GLOBALS['TSFE'], 'search.restrictByRootlinePids');
	...
	protected function _index() {
		...
		if ($this->_index === null) {
			...
			if (TYPO3_MODE == 'FE') {
				Zend_Search_Lucene::setTermsPerQueryLimit(Tx_TwLucenesearch_Utility_Indexer::indexConfig($GLOBALS['TSFE'], 'search.limits.query'));
			}
		}
	...
	protected function _rewriteQueryTerms($searchTerm, &$tokenTerms = array()) {
		...
		try {
			$searchConfig	= Tx_TwLucenesearch_Utility_Indexer::indexConfig($GLOBALS['TSFE'], 'search.searchConfig');
