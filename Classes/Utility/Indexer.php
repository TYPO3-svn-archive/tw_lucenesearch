 ...
 */
class Tx_TwLucenesearch_Utility_Indexer implements t3lib_Singleton {
	/**
	 * Charset object
	 * 
	 * @var t3lib_cs
	 ...
	 * 
	 * @param tslib_fe $fe			Frontend engine
	 * @return void
	 */
	protected function _indexTSFE(tslib_fe $fe) {
		...
		if (!intval($fe->page['no_search']) && self::indexConfig($fe, 'enable')) {
			...
			$indexerService				= t3lib_div::makeInstanceService('index', 'lucene');
			if ($indexerService instanceof t3lib_svbase) {
				...
				$this->_csObj			= t3lib_div::makeInstance('t3lib_cs');
	 ...
	 */
	public function _getPageReference(tslib_fe $fe) {
		$reference					= array();
		foreach (self::indexConfig($fe, 'reference') as $key => $config) {
			$referenceVariable		= $this->_getReferenceVariable(t3lib_div::array_merge_recursive_overrule($_GET, $_POST), $key, $config);
	 ...
	 */
	protected function _getRootLine(tslib_fe $fe) {
		$rootLine					= array();
	 ...
	 */
	protected function _getPageBodytext(tslib_fe $fe) {
		$content					= $fe->content;
	 ...
	 */
	protected function _getPageTitle(tslib_fe $fe) {
		$title			= '';
	 ...
	 */
	protected function _getPageAbstract(tslib_fe $fe, $abstract = '') {
	 ...
	 */
	protected function _getPageKeywords(tslib_fe $fe) {
		$rawKeywords			= '';
	 ...
	 */
	protected function _getPageTimestamp(tslib_fe $fe) {
	 ...
	 */
	protected function _getMetaContent(tslib_fe $fe, $meta, &$metaTag = null) {
	 ...
	 * @return Tx_TwLucenesearch_Domain_Model_Document		Lucene index document
	 */
	protected function _createDocument($properties) {
		$document				= new Tx_TwLucenesearch_Domain_Model_Document();
	 ...
	 */
	public static function indexConfig(tslib_fe $fe, $key = null) {
		if (self::$_config === null) {
			...
			if (isset($fe->tmpl) && ($fe->tmpl instanceof t3lib_TStemplate) && is_array($fe->tmpl->setup) && array_key_exists('config.', $fe->tmpl->setup) && is_array($fe->tmpl->setup['config.'])) {
				...
				if (strlen($indexReference) || strlen($linkVars)) {
					$referenceVars		= array();
					foreach (t3lib_div::trimExplode(',', $linkVars.','.$indexReference) as $referenceVar) {
						if (strlen($referenceVar) && preg_match("%([a-zA-Z][^\(\s]*)(?:\s*\(([^\)]*)\))?(?:\s*\=\s*(.+))?%", trim($referenceVar), $referenceVarConfig)) {
							...
							if ((count($referenceVarConfig) > 2) && strlen($referenceVarConfig[2])) {
								...
								foreach (t3lib_div::trimExplode('|', trim($referenceVarConfig[2])) as $constraint) {
									if (strpos($constraint, '-') !== false) {
										list($lower, $upper) = t3lib_div::trimExplode('-', $constraint);
					...
					if (array_key_exists('search_lucene.', $fe->tmpl->setup['config.']) && is_array($fe->tmpl->setup['config.']['search_lucene.'])) {
						if (array_key_exists('restrictByRootlinePids', $fe->tmpl->setup['config.']['search_lucene.'])) {
							...
							self::$_config['search']->restrictByRootlinePids	= strlen(self::$_config['search']->restrictByRootlinePids) ? t3lib_div::trimExplode(',', self::$_config['search']->restrictByRootlinePids) : array();
						}
						...
						if (array_key_exists('searchConfig', $fe->tmpl->setup['config.']['search_lucene.']) && strlen(trim($fe->tmpl->setup['config.']['search_lucene.']['searchConfig']))) {
							...
							foreach (t3lib_div::trimExplode(',', trim($fe->tmpl->setup['config.']['search_lucene.']['searchConfig'])) as $searchConfig) {
