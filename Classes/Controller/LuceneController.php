 ...
 */
class Tx_TwLucenesearch_Controller_LuceneController extends Tx_Extbase_MVC_Controller_ActionController {
	...
	public function resultsAction($searchterm = '', $pointer = 0, $notfound = false) {
		...
		$this->settings		= Tx_TwLucenesearch_Utility_Indexer::indexConfig($GLOBALS['TSFE']);
		$indexInfo			= null;
		$hits				= array();
		$error				= false;
		$query				= null;
		...
		if ($indexerService instanceof t3lib_svbase) {
			...
			if (!($hits instanceof Tx_TwLucenesearch_Domain_Model_QueryHits)) {
				$error		= true;
			}
		}
